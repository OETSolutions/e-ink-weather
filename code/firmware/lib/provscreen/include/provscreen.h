...
/* Render the screen, with the brand badge drawn into the header.
 *
 * THE BADGE IS PASSED IN rather than fetched here, because it is a firmware asset (generated by
 * tools/gen_boot_logo.py) and this library must stay free of firmware globals to remain
 * host-testable. It is passed to THIS call rather than drawn separately by the caller because
 * the render fills the page white first — a badge drawn beforehand would be erased, and that
 * ordering bug is exactly the kind that looks like "the logo just does not show up".
 *
 * `badge` is 1bpp in the framebuffer convention (bit SET = white), matching canvas_blit_1bpp.
 * A NULL badge renders the screen without it. */
