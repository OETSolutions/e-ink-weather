...
empty glyphs to BOTH faces — over 100 KB of flash. The side table costs one entry per
...
def main():
    ...
    # Index i here means EXTRA_GLYPH_COUNT slots past the end of the ASCII table.
    extra = []
    for spec in (args.extra or ""):
        cp = ord(spec)
        if cp < FIRST_CHAR or cp > LAST_CHAR:
            g = rasterise(font, spec, args.px, ascent)
            if g is None:
                sys.exit(f"failed to rasterise {spec!r} (U+{cp:04X})")
            rows, w, h, adv, bx, by = g
            off = len(blob)
            blob.extend(rows)
    ...
    with open(args.out, "w") as fh:
        ...
        fh.write("};\n\n")
        if extra:
            fh.write(f"#define {args.name}_EXTRA_COUNT {len(extra)}\n")
            fh.write("/* Non-ASCII glyphs, indexed from the END of the ASCII table: the glyph\n"
                     " * index for a codepoint stored here is (codepoint - ASCII_FIRST_CHAR) +\n"
                     " * (codepoint > ASCII_LAST_CHAR ? count - 1 : 0). See font_glyph(). */\n")
            fh.write(f"typedef struct {{ uint32_t codepoint; uint32_t off; uint8_t w, h; "
                     f"uint8_t advance; int8_t bx, by; }} {args.name.lower()}_extra_t;\n\n")
            fh.write(f"static const {args.name.lower()}_extra_t "
                     f"{args.name}_EXTRA[{args.name}_EXTRA_COUNT] = {{\n")
            for (cp, off, w, h, adv, bx, by) in extra:
                fh.write(f"    {{ {cp}u, {off}u, {w}, {h}, {adv}, {bx}, {by} }},\n")
            fh.write("};\n\n")
