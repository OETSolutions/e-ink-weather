...
for x,y in PTS:
    ...
    # 4. Chassis boss must exist and be open on the same axis.
    ...
    ok=(drv_block<0.6 and cb_fill<0.05 and th_fill<0.05 and boss_mat>200.0
        and ch_fill<0.10 and nut_fill<0.10)
    print(' (%.1f,%.1f) driver %.3f cbores %.2f bezel_hole %.2f boss %.0f ch_hole %.2f nut %.2f %s'%
          (x,y,drv_block,cb_fill,th_fill,boss_mat,ch_fill,nut_fill,'OK' if ok else 'FAIL'))
